...
| public       | 公共资源目录                                                 |
| src          | 这里是我们要开发的目录，基本上要做的事情都在这个目录里。里面包含了几个目录及文件：<br/>assets: 静态资源文件，放置一些图片，如logo等。<br/>components: 目录里面放了一个公共组件文件，可以不用。<br/>router:前端路由文件。<br/>store:应用级数据文件。<br/>views:页面目录文件。<br/>App.vue: 项目入口文件，我们也可以直接将组件写这里，而不使用 components 目录。<br/>main.ts: 项目的核心文件(main.js同理)。<br/> |
| test         | 初始测试目录，可删除。                                       |
...
- 此处是页面展示的DOM,在里面第一个标签一般是<div></div>
...
//返回值给DOM
export default{
  //随便自定义,一般和当前vue文件名一致,用于在父组件中导包注册
  name: 'HelloWord',
  //给DOM数据
  data() {
    return {
...
- 样式 //scoped 代表只对当前vue文件有效======主组件不添加,对全局都有效
```vue
<style scoped>
...
import HelloWord from "@/assets/HelloWord";
...
- 在`<script>`里导包
- 在components:{} 注册,多个以 " , " 号隔开
- 在`<template>`中使用,以 <注册的包名>

```vue
<template>
  <div id="app">
    <!--alt: 表示后面图片问题了,给页面显示一个值-->
    <img alt="图片丢失了" src="./assets/logo.png">
    <HelloVue/>
    <!-- 使用   -->
    <HelloWord/>
  </div>
...
module.exports = defineConfig({
    ...
    devServer: {
        ...
        proxy: {
            '/api': {
                target: 'http://127.0.0.1:3333',  // 后台接口域名
...
<li>标签可以用来定义列表，使用 <li> 标签定义的列表可以是个无序列表也可以是有序列表。
...
<input v-model="name" @keydown.enter="addData()" placeholder="编辑wo....."/>  //placeholder 在input输入框中有个提示词
```
