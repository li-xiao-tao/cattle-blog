pipeline {
    ...
    parameters {
        choice(name: 'GITLAB_BRANCH', choices: ['master', 'develop'], description: 'checkout gitlab branch')
    }
    environment {
    ...
    stages {
        stage('checkout') {
            ...
            steps {
                git (credentialsId: "${GITHUB_USER_ID}", url: 'https://github.com/Hopetree/hao.git', branch: "${GITLAB_BRANCH}")
            }
        }
        stage('docker build') {
            steps {
